...
dp = Dispatcher()
agent_log = setup_logger("tg_bot", "agent.log")

# ---------- Runtime Execution Locks (Memory only) ----------
...
def _is_duplicate(text: str) -> bool:
    ...
    sent_prefixes = state.get_val("sent_text_prefixes", {})
    if prefix in sent_prefixes:
        return True
        
    for sent_prefix, sent_full in sent_prefixes.items():
        if text in sent_full:
            return True
        if sent_full in text:
            sent_prefixes[sent_prefix] = text
            state.set_val("sent_text_prefixes", sent_prefixes)
            return True
    return False
...
def _mark_sent(text: str):
    ...
    if prefix:
        sent_prefixes = state.get_val("sent_text_prefixes", {})
        sent_prefixes[prefix] = text
        state.set_val("sent_text_prefixes", sent_prefixes)

def _reset_sent_texts():
    state.set_val("sent_text_prefixes", {})
    state.set_val("bg_baseline_hashes", [])
    state.set_val("bg_last_hash", None)
...
async def get_current_messages() -> list[dict]:
    try:
        ...
        agent_log.error(f"Failed to get snapshot: {e}")
        return []
...
async def poll_for_response(chat_id: int, timeout: float = None):
    global _polling_active, _waiting_for_response
    ...
    last_snapshot_hash = await get_snapshot_hash()
    baseline = await get_current_messages()
    
    baseline_hashes = set(m["hash"] for m in baseline)
    for m in baseline:
        _mark_sent(m["text"])

    prev_hash = last_snapshot_hash
    ever_sent = False
    stable_polls = 0
    ...
    try:
        while time.time() - start_time < effective_timeout:
            ...
            for msg in new_msgs:
                ...
                _mark_sent(text)
                agent_log.info(f"AI_RESPONSE: {text[:100]}...")
                ever_sent = True
        ...
        try:
            ...
            for msg in final_msgs:
                ...
                await send_long_message(chat_id, f"\U0001f916 {text}")
                _mark_sent(text)
                agent_log.info(f"AI_RESPONSE_FINAL: {text[:100]}...")
                ever_sent = True
        except Exception:
            pass

        if not ever_sent:
            agent_log.warning("POLL_TIMEOUT: No AI response received during polling")

    except Exception as e:
        agent_log.error(f"POLL_ERROR: {str(e)}")
        await bot.send_message(chat_id, f"Polling error: {e}")
    finally:
        _polling_active = False
        _waiting_for_response = False
        state.set_val("bg_last_hash", prev_hash)
        state.set_val("bg_baseline_hashes", list(baseline_hashes))
...
async def background_watcher():
    global _bg_watcher_running

    if _bg_watcher_running:
        return
    _bg_watcher_running = True

    agent_log.info("Background watcher started")
    await asyncio.sleep(5)

    try:
        while True:
            await asyncio.sleep(BG_WATCH_INTERVAL)
            active_chat_id = state.get_val("active_chat_id")
            
            if _polling_active or active_chat_id is None:
                continue
            ...
            bg_last_hash = state.get_val("bg_last_hash")
            if current_hash is None or current_hash == bg_last_hash:
                continue

            state.set_val("bg_last_hash", current_hash)
            ...
            bg_baseline_hashes = set(state.get_val("bg_baseline_hashes", []))
            new_msgs = [m for m in current_msgs if m["hash"] not in bg_baseline_hashes]
            if not new_msgs:
                continue

            for msg in new_msgs:
                bg_baseline_hashes.add(msg["hash"])
                state.set_val("bg_baseline_hashes", list(bg_baseline_hashes))
                ...
                try:
                    await send_long_message(active_chat_id, f"\U0001f4e1 \U0001f916 {text}")
                    _mark_sent(text)
                    agent_log.info(f"BG_MSG: {text[:100]}...")
                except Exception as e:
                    agent_log.error(f"BG_SEND_ERROR: {str(e)}")

    except asyncio.CancelledError:
        agent_log.info("Background watcher stopped")
    except Exception as e:
        agent_log.error(f"BG_WATCHER_ERROR: {e}")
    finally:
        _bg_watcher_running = False
...
async def fetch_available_models() -> list[str]:
    ...
    try:
        ...
        agent_log.warning(f"Failed to fetch models: {e}")
...
async def cmd_start(message: types.Message):
    if not is_allowed(message.from_user.id):
        return
    state.set_val("active_chat_id", message.chat.id)
    ...
    await message.answer(
        ...
        "*Обновить* — мгновенно подтянуть новые ответы агента\n"
...
async def show_status(message: types.Message):
    ...
    try:
        app_state = await pw_get("/app-state")
        mode = app_state.get("mode", "Unknown")
...
async def btn_refresh(message: types.Message):
    ...
    try:
        ...
        if not current_msgs:
            await message.answer("Чат пуст или недоступен.", reply_markup=get_main_menu())
            return
        ...
        if not new_msgs:
            await message.answer("🔄 Нет новых сообщений.", reply_markup=get_main_menu())
            return
        ...
        if sent_count == 0:
            await message.answer("🔄 Были только внутренние размышления агента. Значимых ответов пока нет.", reply_markup=get_main_menu())
            
    except Exception as e:
        agent_log.error(f"REFRESH_ERROR: {str(e)}")
        await message.answer(f"Ошибка при обновлении: {e}", reply_markup=get_main_menu())
...
async def btn_new_chat(message: types.Message):
    ...
    try:
        ...
        if result.get("success") or result.get("method"):
            _reset_sent_texts()
            agent_log.info("SYSTEM: New chat started via Telegram.")
...
async def cb_project_launch(callback: types.CallbackQuery):
    ...
    agent_log.info(f"Launching project: {project['name']} at {project['path']}")
...
async def cb_project_add(callback: types.CallbackQuery, state_fsm: FSMContext):
    if not is_allowed(callback.from_user.id):
        return
    await callback.message.answer(
    ...
    await state_fsm.set_state(AddProjectStates.waiting_for_name)
    await callback.answer()

@dp.message(AddProjectStates.waiting_for_name, Command("cancel"))
async def cancel_add_project(message: types.Message, state_fsm: FSMContext):
...
async def process_new_project_name(message: types.Message, state_fsm: FSMContext):
    if not is_allowed(message.from_user.id):
        return
    ...
    agent_log.info(f"Added project: {name}")
...
async def cb_project_remove(callback: types.CallbackQuery):
    ...
    if removed:
        await callback.message.edit_text(f"Project removed from list: {name}\n(files not deleted)")
        agent_log.info(f"Removed project from list: {name}")
    else:
...
async def cb_project_scan(callback: types.CallbackQuery):
    ...
    if added:
        ...
        agent_log.info(f"Scanned and added: {', '.join(added)}")
    else:
...
async def fetch_and_send_artifact(message: types.Message, endpoint: str, filename: str, label: str):
    ...
    agent_log.info(f"User requested {filename}.")
...
async def handle_prompt(message: types.Message):
    ...
    button_texts = {
        "Prompt", "Обновить", "Plan", "Task", "Walkthrough",
    ...
    agent_log.info(f"USER_PROMPT: {text[:200]}")
    await bot.send_chat_action(message.chat.id, "typing")
    ...
    try:
        ...
        agent_log.error(f"SEND_ERROR: {str(e)}")
        await message.answer(
